 ...
 */
class BasePolygon extends AbstractPolygon implements
		IPolygon {
	...
	private List<Point> defensiveCopyPointList(
			List<Point> points) {
	...
	private static Point calculateCenterPoint(
			List<Point> points) {
		...
		return new Point((maxX - minX) / 2 + minX,
				(maxY - minY) / 2 + minY);
	}
